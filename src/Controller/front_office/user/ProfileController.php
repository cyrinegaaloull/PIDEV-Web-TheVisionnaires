...
if (!empty($newPassword)) {
    ...
    $errors['confirm_password'] = 'Please enter the new password if you wish to change it.'; // Keep consistent
            ...
            // --- Avatar Validation (Optional: Add checks for size, mime type if needed) ---
            if ($avatarFile) {
                 $allowedMimes = ['image/jpeg', 'image/png', 'image/gif'];
                 if (!in_array($avatarFile->getMimeType(), $allowedMimes)) {
                     $errors['avatar'] = 'Invalid file type. Only JPEG, PNG, and GIF are allowed.';
                 }

                 // Example Size Check (e.g., max 2MB)
